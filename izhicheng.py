...
url = 'http://dw10.fdzcxy.edu.cn/datawarn/ReportServer?formlet=app/sjkrb.frm&op=h5&userno=212006165#/form'
...
def post_form(JsConfId,CallbackConfId,now_time,cookie,sessionID):
    headers = {
        '__device__': 'android',
        'Accept': 'application/json, text/plain, */*',
        'Accept-Encoding': 'gzip, deflate',
    ...
    data = {
        ...
        '{"jsConfId":' +JsConfId+', "callbackConfId":' +CallbackConfId+','
         '"LABEL2": "  每日健康上报", "XH":'+ sno+', "XM":'+ name+', "LABEL12": "", "LABEL0": "1. 目前所在位置:", "SHENG": "350000",'
         '"SHI": "350100", "QU": "350121", "LABEL11": "2.填报时间:", "SJ":'+ now_time+','
         '"LABEL1": "3. 今日体温是否正常？(体温小于37.3为正常)", "TWZC": "正常", "LABEL6": "目前体温为：", "TW": "0", "TXWZ": "福建省福州市闽侯县",'
...
try:
    ...
    post_form(JsConfId,CallbackConfId,now_time,cookie,sessionID)
    print('{0}你自动填报的时间是:{1}'.format(name,now_time))
except:
    print("填报错误请再次试试")

with open('health.csv','w') as f:
    writer = csv.writer(f)
    writer.writerow(['{0}你自动填报的时间是:{1}'.format(name,now_time)])
